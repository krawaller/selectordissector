...
export function describeError(error: QueryError): string {
  switch(error){
    ...
    case QueryError.hasPseudoSelector: return `We haven't implemented the :has selector`;
    case QueryError.isPseudoSelector: return `We haven't implemented the :is selector`;
    ...
    case QueryError.unImplemented: return `We haven't implemented this pseudo selector`;
